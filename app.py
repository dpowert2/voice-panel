...
import pipecat_panel  # noqa: E402  # cartesia-agents mode (Pipecat pipeline)
...
# ---------------------------------------------------------------------------
# Cartesia-agents mode — Pipecat pipeline
# ---------------------------------------------------------------------------
# The cartesia-agents provider routes each panel turn through pipecat_panel.
# Pipecat owns: text -> CartesiaTTSService(voice=<persona-specific>) -> raw PCM.
# We own: persona selection (via director.orchestrate), transcript management,
# and the aiohttp chunked-transfer delivery to the browser.
#
# Three personas are active in this mode: doctor / holistic / lawyer. Each is
# permanently bound to its own cartesia_voice_id, exactly as the
# `local-handoff-two-agents-tts.py` reference pattern recommends.
...
async def _stream_tts(
    ...
    if provider == "cartesia-agents":
        # Pipecat-backed path: Claude writes a 1-2 sentence reply, Pipecat's
        # CartesiaTTSService (voice permanently bound to this persona) renders
        # it as raw PCM. We don't use the streaming Anthropic iterator here —
        # pipecat_panel makes its own Claude call so it can return the full
        # text for transcript appending. The anthropic_iter passed in is left
        # untouched (the chain runner never awaits it for this provider).
        panel_context = "\n".join(_transcript[-12:])
        text_out: list[str] = []
        async for chunk in pipecat_panel.agents_mode_stream(
            persona,
            panel_context,
            ...
            yield chunk
        # Propagate the reply text up to the chain via the shared text_buf
        # (the chain runner reads "".join(text_buf) for transcript update).
        if text_out:
            text_buf.append(text_out[0])
        return
...
async def _run_chain(forced_first_speaker: str | None = None) -> None:
    ...
    try:
        for i in range(MAX_CHAIN_TURNS):
            ...
            # the streaming providers append it token by token.
...
async def on_startup(app: web.Application) -> None:
    ...
    # Log whether Pipecat is available for the cartesia-agents path.
    if pipecat_panel.pipecat_available():
        log.info("pipecat available — cartesia-agents will use Pipecat pipeline")
    else:
        log.warning(
            "pipecat not available (%s) — cartesia-agents will use fallback WS",
            pipecat_panel.pipecat_import_error(),
        )
